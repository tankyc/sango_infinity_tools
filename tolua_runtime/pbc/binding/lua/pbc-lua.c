...
_rmessage_int64(lua_State *L) {
	...
	int64_t v64 = (int64_t)((uint64_t)hi << 32 | (uint64_t)low);
	if (toluaflags & FLAG_INT64)
	{
		lua_pushinteger(L, (lua_Integer)v64);
	}
	else
	{
...
_rmessage_uint64(lua_State* L) {
	...
	uint32_t hi, low;
	low = pbc_rmessage_integer(m, key, index, &hi);
	uint64_t v64 = (uint64_t)((uint64_t)hi << 32 | (uint64_t)low);
	if (toluaflags & FLAG_INT64)
	{
		lua_pushinteger(L, (lua_Integer)v64);
	}
...
_wmessage_int64(lua_State* L) {
	...
	case LUA_TUSERDATA: {
		int64_t v64 = *(int64_t*)lua_touserdata(L, 3);
		pbc_wmessage_integer(m, key, (uint32_t)(v64 & 0xFFFFFFFF), (uint32_t)(v64 >> 32));
		break;
	}
	case LUA_TNUMBER: {
...
_wmessage_uint64(lua_State* L) {
	...
	case LUA_TUSERDATA: {
		uint64_t v64 = *(uint64_t*)lua_touserdata(L, 3);
		if (v64 < 0)
			return luaL_error(L, "negative number : %lld passed to unsigned field", v64);
		pbc_wmessage_integer(m, key, (uint32_t)(v64 & 0xFFFFFFFF), (uint32_t)(v64 >> 32));
		break;
	}
	case LUA_TNUMBER: {
		uint64_t number = (uint64_t)(luaL_checknumber(L, 3));
		uint32_t hi = (uint32_t)(number >> 32);
		pbc_wmessage_integer(m, key, (uint32_t)number, hi);
		break;
	}
